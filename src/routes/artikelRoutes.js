// routes/artikelRoutes.js
const express = require("express");
const router = express.Router();
const artikelController = require("../controllers/artikelController");
const upload = require("../middleware/uploadFileMiddleware");
const {
    validateUploadFile,
} = require("../middleware/validationUploadFileMiddleware");

const validate = require("../middleware/validationMiddleware");
const {
    createSchema,
    updateSchema,
} = require("../validators/artikelValidation");

// Auth
const {
    authenticateAdminToken,
} = require("../middleware/authenticateAdminToken");

router.post(
    "/",
    authenticateAdminToken(["Super Admin", "Admin", "Public Relation"]),
    upload.any(), // semua file akan ditampung di req.files
    validateUploadFile({ fieldName: "gambar" }), // validasi khusus gambar utama
    validate(createSchema),
    artikelController.create,
);

router.put(
    "/:id",
    authenticateAdminToken(["Super Admin", "Admin", "Public Relation"]),
    upload.any(), // semua file akan ditampung di req.files
    validateUploadFile({
        fieldName: "gambar",
        required: false,
    }),
    validate(updateSchema),
    artikelController.update,
);
router.get("/", artikelController.findAll);
router.get("/:slug", artikelController.findDataBySlug);
router.delete(
    "/:id",
    authenticateAdminToken(["Super Admin", "Admin", "Public Relation"]),
    artikelController.delete,
);
router.get("/image/:gambar", artikelController.getImageByName);
router.get(
    "/imageSubArtikel/:gambar",
    artikelController.getImageSubArtikelByName,
);
router.get("/filter/tag", artikelController.getArticleByTag);

module.exports = router;
